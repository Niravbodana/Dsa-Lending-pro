...
import { EDITABLE_LAYERS } from "@/lib/visual-editor/types";
    ...
    <aside className="flex w-72 shrink-0 flex-col border-l border-slate-700 bg-slate-900 text-white">
      <div className="border-b border-slate-700 px-4 py-3">
        ...
        <p className="mt-1 text-sm text-slate-300">
          {path ? (isBlock ? "Custom text" : path) : "Click any element on the page"}
        </p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {!path && (
          <p className="text-xs text-slate-400">
            Select text, buttons, or images on the preview. Use <strong>Move</strong> tool to drag.
          </p>
        )}
        ...
        {path && !isBlock && stylePath && (
          <>
            ...
            {stylePath === "hero.image_url" && (
              <label className="block text-xs font-semibold text-slate-400">
                Image URL
                <input
                  value={fieldValue}
                  onChange={(e) => ctx.updateField(stylePath, e.target.value)}
                  className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-800 px-3 py-2 text-xs"
                />
              </label>
            )}
          </>
          ...
          <ul className="mt-2 max-h-48 space-y-1 overflow-y-auto text-xs">
            {EDITABLE_LAYERS.map((layer) => (
              <li key={layer.path}>
                ...
                >
                  <span className="text-[10px] text-slate-500">{layer.group}</span>
                  <br />
                  {layer.label}
                </button>
              </li>
